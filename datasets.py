...
class PanopticSet(data.Dataset):
    def __init__(self, root, dataset_list, image_size, data_par='train', img_transform=None):
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        param = self.adjust_param(torch.Tensor(np.loadtxt(gt_path)), original_mask.size[0])
